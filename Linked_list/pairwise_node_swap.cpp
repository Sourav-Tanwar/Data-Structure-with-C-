/* PROGRAM FOR INSERTION, DELETION AND DISPLAY IN LINKED LIST*/
...
node* pairwise_node_swap_recursively(node *Head)
{
    node *pending , *t;
    ...
    pending = Head->next->next;
    t = Head->next;
    Head->next->next = Head;
    Head ->next = pairwise_node_swap_recursively(pending);
    return(t);

}


void pairwise_node_swap_iterative(node **Head)
{
    node *prev , *curr, *t;
    if(!(*Head) || !((*Head)->next))
    {
...
int main() {
    ...
    cout<<"\n\t--------------------------------";
    cout<<"\n\t 1.INSERTION\n\t 2.DELETITION\n\t 3.DISPLAY\n\t 4.EXIT\n\t 5.PAIRWISE SWAP RECUSIVELY\n\t 6.PAIRWISE SWAP ITERATIVELY"<<endl;
    ...
    do
    {
        ...
        switch(i){
            ...
            case 5: Head = pairwise_node_swap_recursively(Head);
                    cout<<"Pairwise swap is done."<<endl;
                    break;
            case 6: 
                    pairwise_node_swap_iterative(&Head);
                    cout<<"Pairwise swap is done."<<endl;
                    break;
            default:
